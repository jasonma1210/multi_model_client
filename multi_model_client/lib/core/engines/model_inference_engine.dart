...
class ModelInferenceEngine implements IModelManager {
  ...
  @override
  Future<ModelInfo> loadModel(String modelId) async {
    try {
      ...
      if (modelEntry.isLocal) {
        ...
        return await _loadLocalModelViaFFI(modelEntry);
      } else {
  ...
  Future<ModelInfo> _loadLocalModelViaFFI(ModelEntry modelEntry) async {
    try {
      // 缓存本地模型参数
      ...
      return ModelInfo(
        ...
        config: {'modelPath': modelEntry.filePath},
      );
    } catch (e) {
  ...
  bool isModelReady(String modelId) {
    return _ollamaReadyModels.contains(modelId) ||
  ...
  Future<String> generateChat(
    ...
    if (modelEntry.isLocal) {
      if (!_ollamaReadyModels.contains(modelId)) {
        ...
        throw StateError(
            '本地模型 ${modelEntry.displayName} 未加载，请先在模型页面加载模型');
      }
      return await _callOllamaChat(modelId, messages,
          options: options ?? _cachedOptions[modelId]);
    } else {
      final config = modelEntry.remoteConfig;
      if (config == null) {
      ...
      final effectiveOptions =
          options ?? _cachedOptions[modelId] ?? ChatOptions.fromRemoteConfig(config);

      switch (config.protocol) {
        case RemoteProtocol.openai:
          return await _callOpenAI(dio, config, messages, effectiveOptions);
        case RemoteProtocol.anthropic:
          return await _callAnthropic(
              dio, config, messages, effectiveOptions);
        case RemoteProtocol.ollama:
          return await _callOllamaRemoteAPI(
              dio, config, messages, effectiveOptions);
      }
    }
  }
  ...
  Stream<String> generateChatStream(
    ...
    if (modelEntry.isLocal) {
      ...
      yield* _streamOllamaChat(modelId, messages,
          options: options ?? _cachedOptions[modelId]);
    } else {
      final config = modelEntry.remoteConfig;
      if (config == null) {
      ...
      switch (config.protocol) {
        ...
        case RemoteProtocol.anthropic:
          yield* _streamAnthropic(
              modelId, config, messages, effectiveOptions);
        case RemoteProtocol.ollama:
          yield* _streamOllamaRemoteAPI(
              modelId, config, messages, effectiveOptions);
      }
    }
  }
  ...
  Future<String> _callAnthropic(
    ...
    final data = <String, dynamic>{
      ...
      'messages': nonSystemMessages.map((m) => m.toJson()).toList(),
    };
  ...
  void recordGenerationTime(String modelId, int elapsedMs, int outputLength) {
    ...
    while (history.length > _maxHistorySize) history.removeAt(0);
  }
